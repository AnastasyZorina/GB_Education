const FIELD_SIZE_X = 20;    //Размер поля по оси Х
const FIELD_SIZE_Y = 20;    //Размер поля по оси Y
...
let barrierTimer;  //Таймер препятствий
let barrierCreationSpeed = 5000;   //Скорость появления препятствий
let numberOfBarrier = [5, 0] // Количество препятствий на поле
...
function haveFood(unit){
	...
	if(unitClasses.includes("food-unit")){
        ...
        score++;
        console.log(score);
        //выводим счет в реальном времени
...
function createBarrier() {
    if (numberOfBarrier[1] < numberOfBarrier[0]) {
        ...
        while (!barrierCreated) {
            ...
            //Если тут нет змейки, то размещаем препятствие
            if (!barrierCellClasses.includes("snake-unit") || !barrierCellClasses.includes("food-unit")) {
                ...
                console.log(numberOfBarrier);
            }
        }
        //Убираем 1ый барьер
    }   else {
        numberOfBarrier[1]--;
        let barrierUnit = document.getElementsByClassName("barrier-unit")[0];
        let unitClasses = barrierUnit.getAttribute("class").split(" ");
        console.log(unitClasses);
        barrierUnit.setAttribute("class", unitClasses[0] + " " + unitClasses[1]);
